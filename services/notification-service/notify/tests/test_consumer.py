...
def test_payment_success_creates_one_notification_and_is_idempotent():
    ...
    notes = Notification.all_objects.filter(tenant_id=tenant_id, user_code=student_user_code)
    ...
    dispatch_event(event)
    assert (
        Notification.all_objects.filter(tenant_id=tenant_id, user_code=student_user_code).count()
        == 1
    )
...
def test_grievance_scored_with_raised_by_creates_notification():
    tenant_id = uuid.uuid4()
    owner_code = "STU-100"
    event = build_event(
        "grievance.scored",
    ...
    notes = Notification.all_objects.filter(tenant_id=tenant_id, user_code=owner_code)
    assert notes.count() == 1
    note = notes.first()
    assert note.title == "Grievance update"
    assert "critical" in note.body
...
def test_grievance_scored_without_recipient_is_skipped_gracefully():
    ...
    # No recipient code in the payload -> skipped without raising.
    dispatch_event(event)
