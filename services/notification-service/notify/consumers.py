...
def handle_payment_success(event: dict) -> None:
    ...
    Payload: ``{invoice_id, student_user_code, purpose, amount}``.
    """
...
def handle_allocation_confirmed(event: dict) -> None:
    ...
    Payload: ``{allocation_id, student_user_code, room_id}``.
    """
    tenant_id = event["tenant_id"]
    payload = event["payload"]
    student_user_code = payload["student_user_code"]
...
def handle_grievance_scored(event: dict) -> None:
    ...
    Payload: ``{ticket_id, raised_by, sentiment, urgency}``.

    ``raised_by`` is the recipient user_code echoed from grievance.created; it
    may be null if absent upstream, in which case the event is skipped
    gracefully (no notification, no error).
    """
    tenant_id = event["tenant_id"]
    payload = event["payload"]
    recipient_code = payload.get("raised_by")
