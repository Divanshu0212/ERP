...
def test_driver_sees_only_own_schedules_with_booked_count():
    ...
    Booking.all_objects.create(
        tenant_id=tenant_id, schedule=mine, student_id=uuid.uuid4(), seat_no=1, status="booked"
    )
...
def test_admin_sees_all_tenant_schedules():
    tenant_id = uuid.uuid4()
    _schedule(tenant_id, uuid.uuid4(), bus_no="A")
    _schedule(tenant_id, uuid.uuid4(), bus_no="B")
...
def test_schedule_bookings_ownership_enforced():
    ...
    Booking.all_objects.create(
        tenant_id=tenant_id, schedule=schedule, student_id=uuid.uuid4(), seat_no=3, status="booked"
    )
    ...
    resp = _client(tenant_id, user_id=uuid.uuid4(), role="driver").get(
        f"/api/v1/transport/schedules/{schedule.id}/bookings"
    )
    assert resp.status_code == 403
