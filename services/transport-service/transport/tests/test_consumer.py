...
def _payment_success_event(tenant_id, student_id, purpose="transport", **extra):
    payload = {
...
def test_transport_payment_activates_pass():
    ...
    event = _payment_success_event(tenant_id, student_id, purpose="transport")

    handle_payment_success(event)

    passes = Pass.all_objects.filter(tenant_id=tenant_id, student_id=student_id)
...
def test_bus_pass_purpose_also_activates_pass():
    tenant_id = uuid.uuid4()
    student_id = uuid.uuid4()
    event = _payment_success_event(tenant_id, student_id, purpose="bus_pass")

    handle_payment_success(event)

    assert (
        Pass.all_objects.filter(tenant_id=tenant_id, student_id=student_id, active=True).count()
        == 1
    )
...
def test_redelivery_same_event_id_is_idempotent():
    ...
    passes = Pass.all_objects.filter(tenant_id=tenant_id, student_id=student_id, active=True)
    assert passes.count() == 1
...
def test_hostel_purpose_is_skipped_no_pass_created():
    tenant_id = uuid.uuid4()
    student_id = uuid.uuid4()
    event = _payment_success_event(tenant_id, student_id, purpose="hostel")

    handle_payment_success(event)

    assert Pass.all_objects.filter(tenant_id=tenant_id, student_id=student_id).count() == 0
