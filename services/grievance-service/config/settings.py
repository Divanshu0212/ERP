"""Django settings for grievance-service.

grievance-service is a stateless resource service: it verifies JWTs issued by
auth-service (via suerp_common) and owns its own grievance database. It has no
user model of its own — see services/finance-service/config/settings.py for the
canonical template this bootstrap mirrors (suerp_common auth/envelope/tenancy,
env-driven DB/broker config, Celery, Prometheus, drf-spectacular). Auth-only
bits (AUTH_USER_MODEL, SIMPLE_JWT, argon2 hashers) are intentionally absent.
"""
...
SPECTACULAR_SETTINGS = {
    "TITLE": "SU-ERP Grievance Service API",
    "DESCRIPTION": "Student grievance tickets with AI sentiment scoring and auto-escalation.",
    "VERSION": "1.0.0",
...
# service to its own named queue (matched by the "-Q grievance" flag on this
# service's celery command in infra/docker-compose.yml) keeps every worker
# only ever consuming its own tasks.
CELERY_TASK_DEFAULT_QUEUE = "grievance"
...
# low latency. Mirrors finance-service/config/settings.py's own beat schedule,
# under this service's own domain app (grievance) and task name.
