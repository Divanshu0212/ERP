"""Django settings for finance-service.

finance-service is a stateless resource service: it verifies JWTs issued by
auth-service (via suerp_common) and owns its own billing database. It has no
user model of its own — see services/auth-service/config/settings.py for the
...
SPECTACULAR_SETTINGS = {
    "TITLE": "SU-ERP Finance Service API",
    "DESCRIPTION": "Billing, invoices, payments, and receipts for SU-ERP.",
    "VERSION": "1.0.0",
...
# under this service's own domain app (billing) and task name.
CELERY_BEAT_SCHEDULE = {
    "drain-outbox-finance": {
        "task": "billing.drain_outbox_task",
        "schedule": 5.0,
    },
}
